...
type Props = {
  label: string;
  error?: FieldError;
  options: { id: number | string; descripcion: string; categoria?: string }[];
};
...
function Select({ label, error, options, ...props }: Props) {
  return (
    <div className="mt-3" id={label}>
      ...
      <select
        className={
          "bg-gray-50 border border-gray-300 text-gray-900 rounded-lg block w-full p-2.5 cursor-pointer outline-none"
        }
        {...props}
      >
        {options.map((option) => (
