...
const axiosQuery = async (accessToken: string) => {
  return await axios.get("http://localhost:8000/api/informes/entregas_sucursal", {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
};

function useEntregasSucursal() {
  ...
  return useMutation<[ComprasType]>({
    ...
    onSuccess: (data) => {
      ...
      utils.book_append_sheet(wb, ws, "Entregas Sucursal");
      writeFile(wb, "reporte_entregas_sucursal.xlsx"); //descarga el excel con los datos 
    },
  });
}

export default useEntregasSucursal;
