...
const axiosQuery = async (accessToken: string) => {
  return await axios.get("http://localhost:8000/api/informes/stock_productos", {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
};

function useStockProductos() {
  ...
  return useMutation<[ComprasType]>({
    ...
    onSuccess: (data) => {
      ...
      utils.book_append_sheet(wb, ws, "Stock productos");
      writeFile(wb, "reporte_stock_productos.xlsx"); //descarga el excel con los datos 
    },
  });
}

export default useStockProductos;
