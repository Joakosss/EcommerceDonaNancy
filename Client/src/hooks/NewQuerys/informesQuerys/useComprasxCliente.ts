...
const axiosQuery = async (accessToken: string) => {
  return await axios.get("http://localhost:8000/api/informes/compras_por_cliente", {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
};

function useComprasxCliente() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      utils.book_append_sheet(wb, ws, "Compras cliente");
      writeFile(wb, "reporte_compras_cliente.xlsx"); //descarga el excel con los datos 
    },
  });
}

export default useComprasxCliente;
