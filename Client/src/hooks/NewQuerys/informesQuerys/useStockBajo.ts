...
const axiosQuery = async (accessToken: string) => {
  return await axios.get("http://localhost:8000/api/informes/stock_bajo", {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
};

function useStockBajo() {
  ...
  return useMutation<[ComprasType]>({
    ...
    onSuccess: (data) => {
      ...
      utils.book_append_sheet(wb, ws, "Stock Bajo");
      writeFile(wb, "reporte_stock_bajo.xlsx"); //descarga el excel con los datos 
    },
  });
}

export default useStockBajo;
